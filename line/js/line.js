...
// .random() : 0~1사이에 난수 반환
// Math.floor : 소수점 첫째자리에서 내림하면 정수 반환

$(function(){
    var $input = $("input#lang");               // input
...
$(function(){
    ...
    $(window).one("load",function(){
        var strFome = '<form action="#" name="frmLang" method="post"><fieldset><legend>다국어서비스</legend> <label for="langsrv" class="blind">언어선택</label><select name="langsrv" id="langsrv"></select></fieldset></form>';
        $("footer>.fContainer>address").before(strFome);
        // .before 이전위치
        // .each 순차적으로 하나씩 접근
...
$(function(){
    var $mainbanrSwipe=document.getElementById("sliderSwiper");     // swipe영역 : swipe.js를 쓰려면 원래방식의 스크립트 문법으로 써줘야함 (이거 하나만)
    var $indicator=$("#sliderSwiper~.mainbanr-pagination>li>a");
    var nowIdx=Math.floor(Math.random()*4);        // 현재 보이는 슬라이드의 index번호 
    var oldIdx=nowIdx;
...
$(function(){
    ...
    // 1단계 - 메인 슬라이딩과 article의 탑 값을 배열에서 저장
    ...
    function setPosArticle(){
        ...
        $("#mainsrv>article").each(function(idx){
            arrContH.push($(this).offset().top);     // article이 시작하는 y좌표갑을 동적으로 배열에 저장 / article의 높이가 변하더라고 정확하게 시작점으로 스크롤한다.
        });
    };
    ...
    // mobile버젼의 메인 서비스 내용
    var msgText_MO = "1:1대화는 물론 그룹 대화까지<br /> 무제한 무료로 즐겨보세요.";
    ...
    // 640px 이하에서만 나타나는 header영역의 메뉴 버튼
